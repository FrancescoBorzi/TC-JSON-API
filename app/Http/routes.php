...
Route::get('/auction', function() {
    ...
    if (\App\Helpers\TCAPI::is("wod"))
    {
        ...
        FROM ' . config('database.connections.characters.database') . '.auctionhouse AS ah
        LEFT JOIN ' . config('database.connections.characters.database') . '.item_instance AS ins ON ah.itemguid = ins.guid
        LEFT JOIN ' . config('database.connections.characters.database') . '.characters AS c ON ah.itemowner = c.guid
        LEFT JOIN ' . config('database.connections.characters.database') . '.characters AS c2 ON ah.buyguid = c2.guid LIMIT ' . $itemFrom . ' , ' . $numItem);
    }
    else
    {
        ...
        FROM ' . config('database.connections.characters.database') . '.auctionhouse AS ah
        LEFT JOIN ' . config('database.connections.characters.database') . '.item_instance AS ins ON ah.itemguid = ins.guid
        LEFT JOIN ' . config('database.connections.world.database') .'.item_template AS n ON ins.itemEntry = n.entry
        LEFT JOIN ' . config('database.connections.characters.database') . '.characters AS c ON ah.itemowner = c.guid
        LEFT JOIN ' . config('database.connections.characters.database') . '.characters AS c2 ON ah.buyguid = c2.guid LIMIT ' . $itemFrom . ' , ' . $numItem);
...
Route::get('/character_achievement', function() {
    ...
    FROM ' . config('database.connections.characters.database') . '.character_achievement cha
    JOIN ' . config('app.trinity.DB_DBC') . '.achievement AS ac ON cha.achievement = ac.ID
    JOIN ' . config('database.connections.characters.database') . '.characters AS ch ON cha.guid = ch.guid
    GROUP BY (cha.guid)
...
Route::get('/character_achievement/{guid}', function($guid) {
    ...
    if (isset($_GET['category']) && $_GET['category'] != "") {
        $result = DB::select("SELECT ID, Name, Description, Category, Points, icon FROM " . config('app.trinity.DB_DBC') . ".achievement WHERE ID IN (SELECT achievement FROM " . config('database.connections.characters.database') . ".character_achievement WHERE guid = " . $guid . ") AND category = " . $_GET['category']);
    } else {
...
Route::get('/achievement_progress', function() {
	...
	FROM " . config('database.connections.characters.database') . ".character_achievement_progress as ach_progress
	...
	WHERE acc.ID IN (
		SELECT ac.criteria
		FROM " . config('database.connections.characters.database') . ".character_achievement_progress AS ac
		JOIN " . config('database.connections.characters.database') . ".characters AS c ON ac.guid = c.guid
...
Route::get('topgm/ticket/account', function() {
    $results = DB::select('SELECT t3.username AS account, COUNT(*) AS count FROM `' . config('database.connections.characters.database') . '`.gm_ticket AS t1 INNER JOIN `' . config('database.connections.characters.database') . '`.characters AS t2 ON t1.resolvedBy = t2.guid INNER JOIN `' . config('database.connections.auth.database') . '`.account AS t3 ON t2.account = t3.id GROUP BY t3.username ORDER BY count DESC;');
...
Route::get('topgm/ticket/account/month/{diff}', function($diff) {
  ...
  FROM `' . config('database.connections.characters.database') . '`.gm_ticket AS t1 INNER JOIN `' . config('database.connections.characters.database') . '`.characters AS t2 ON t1.resolvedBy = t2.guid
  INNER JOIN `' . config('database.connections.auth.database') . '`.account AS t3 ON t2.account = t3.id
...
Route::get('topgm/ticket/character', function() {
    $results = DB::select('SELECT t2.name, t3.username AS account, COUNT(*) AS count FROM `' . config('database.connections.characters.database') . '`.gm_ticket AS t1 INNER JOIN `' . config('database.connections.characters.database') . '`.characters AS t2 ON t1.resolvedBy = t2.guid INNER JOIN `' . config('database.connections.auth.database') . '`.account AS t3 ON t2.account = t3.id GROUP BY t2.guid ORDER BY count DESC;');
